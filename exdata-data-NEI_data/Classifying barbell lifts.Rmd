...
```{r Getting and preparing the data}
...
```{r calculating columns having missing values}
n <- dim(train.k)[2]
...
```{r subdividing training set to create cross validation set}
library(caret)
...
```{r using classification and regression model}
library(tree)
...
```{r estimating in-sample error continued}
...
```{r estimating out-sample error continued}
error.cv <- {Inf}
for (i in 2:19) {
...
```{r estimate the in-sample error using RPart trees}
...
```{r cross validation after finding in-sample error}
...
```{r in-sample error for random forest}
